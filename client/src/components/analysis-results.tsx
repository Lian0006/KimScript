...
import { Search, File, FileText, ChevronDown, ChevronUp, Copy, Save, Zap, TrendingUp, Brain, Target, Video, Eye, Play, Check, Plus, Clock, BarChart3, Lightbulb, Download, Share2, RefreshCw, Timer, Users, Hash, Star } from "lucide-react";
...
export default function AnalysisResults({ result, isBlocked = false, onNewAnalysis }: AnalysisResultsProps) {
        ...
        <Tabs defaultValue="script" className="space-y-6">
          ...
          <TabsContent value="analysis" className="space-y-6">
            ...
            <div className="grid lg:grid-cols-3 gap-6 mt-6">
              {/* Viral Elements */}
              {currentResult.analysis.viralElements && currentResult.analysis.viralElements.length > 0 && (
                <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                    ...
                    <div className="space-y-2">
                      {currentResult.analysis.viralElements.map((element, index) => (
                        <div key={index} className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                          <p className="text-sm text-gray-800">{element}</p>
                        </div>
                      ))}
                    </div>
                  </CardContent>
              ...
              {/* Viral Mechanics */}
              {currentResult.analysis.viralMechanics && currentResult.analysis.viralMechanics.length > 0 && (
                <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-2">
                      {currentResult.analysis.viralMechanics.map((mechanic, index) => (
              ...
              {currentResult.analysis.contentFramework && (
                <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                  <CardHeader className="bg-gradient-to-r from-teal-600 to-green-600 text-white p-4">
                    <CardTitle className="text-lg font-bold flex items-center">
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      <div className="p-3 bg-teal-50 rounded-lg border border-teal-200">
                        <p className="text-xs font-medium text-teal-600 mb-1">Framework Principal</p>
                        <p className="text-sm font-semibold text-gray-800">{currentResult.analysis.contentFramework.primary}</p>
                      </div>
                      
                      {currentResult.analysis.contentFramework.secondary && currentResult.analysis.contentFramework.secondary.length > 0 && (
                        <div className="p-3 bg-teal-50 rounded-lg border border-teal-200">
                          <p className="text-xs font-medium text-teal-600 mb-1">Frameworks Secundarios</p>
                          <div className="flex flex-wrap gap-1">
                            {currentResult.analysis.contentFramework.secondary.map((framework, index) => (
                              <Badge key={index} variant="outline" className="text-xs bg-teal-100 text-teal-700 border-teal-300">
                                {framework}
                              </Badge>
                            ))}
                          </div>
                      ...
                      <div className="p-3 bg-teal-50 rounded-lg border border-teal-200">
                        <p className="text-xs font-medium text-teal-600 mb-1">Confianza</p>
                        <div className="flex items-center">
                          <div className="w-full bg-gray-200 rounded-full h-2 mr-2">
                            <div 
                              className="bg-teal-600 h-2 rounded-full" 
                              style={{ width: `${currentResult.analysis.contentFramework.confidence}%` }}
                            ></div>
                          </div>
                          <span className="text-sm font-bold text-teal-700">{currentResult.analysis.contentFramework.confidence}%</span>
                        </div>
                      </div>
                    </div>
                  </CardContent>
            ...
            {currentResult.analysis.storytellingStructure && (
              <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden mt-6">
                <CardHeader className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-6">
                  <CardTitle className="text-2xl font-bold flex items-center">
                    <FileText className="h-6 w-6 mr-3" />
                    Estructura Narrativa
                  </CardTitle>
                  <p className="text-indigo-100 mt-2">Análisis de la estructura de storytelling</p>
                </CardHeader>
                <CardContent className="p-6">
                  <div className="grid md:grid-cols-3 gap-6">
                    <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                      <h4 className="font-semibold text-blue-800 mb-2">Inicio</h4>
                      <p className="text-sm text-gray-700">{currentResult.analysis.storytellingStructure.beginning}</p>
                    </div>
                    <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                      <h4 className="font-semibold text-yellow-800 mb-2">Desarrollo</h4>
                      <p className="text-sm text-gray-700">{currentResult.analysis.storytellingStructure.middle}</p>
                    </div>
                    <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                      <h4 className="font-semibold text-green-800 mb-2">Final</h4>
                      <p className="text-sm text-gray-700">{currentResult.analysis.storytellingStructure.end}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="grid lg:grid-cols-2 gap-6 mt-6">
              ...
              {currentResult.analysis.keyPhrases && currentResult.analysis.keyPhrases.length > 0 && (
                <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                  <CardHeader className="bg-gradient-to-r from-cyan-600 to-blue-600 text-white p-6">
                    ...
                    <p className="text-cyan-100 mt-1">Momentos importantes con timestamps</p>
                  </CardHeader>
                  <CardContent className="p-6">
                    <div className="space-y-3">
                        ...
                        <div key={index} className="p-4 bg-cyan-50 rounded-lg border border-cyan-200">
                          <div className="flex items-start justify-between mb-2">
                            <Badge variant="outline" className="text-xs bg-cyan-100 text-cyan-700 border-cyan-300">
                              {phrase.start_s}s - {phrase.source}
                            </Badge>
                          </div>
                          <p className="text-sm text-gray-800 font-medium italic">"{phrase.quote}"</p>
                        </div>
                      ))}
                    </div>
                  </CardContent>
              ...
              <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white p-6">
                  ...
                  <p className="text-emerald-100 mt-1">Call to Action y predicciones</p>
                </CardHeader>
                <CardContent className="p-6">
                  <div className="space-y-4">
                    {currentResult.analysis.cta && (
                      <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                        <p className="text-sm font-medium text-emerald-600 mb-2">Call to Action Original</p>
                        <p className="text-sm text-gray-800 font-medium">{currentResult.analysis.cta}</p>
                      </div>
                    )}
                    
                    {currentResult.analysis.viralPotential && (
                      <div className="p-4 bg-orange-50 rounded-lg border border-orange-200">
                        <p className="text-sm font-medium text-orange-600 mb-2">Potencial Viral</p>
                        <Badge className={`${currentResult.analysis.viralPotential === 'alto' ? 'bg-red-600' : currentResult.analysis.viralPotential === 'medio' ? 'bg-yellow-600' : 'bg-green-600'} text-white`}>
                          {currentResult.analysis.viralPotential.toUpperCase()}
                        </Badge>
                      </div>
                    )}
                    ...
                    {currentResult.analysis.engagementPrediction && (
                      <div className="space-y-3">
                        <p className="text-sm font-medium text-gray-700">Predicciones de Engagement:</p>
                        <div className="grid grid-cols-3 gap-2">
                          <div className="p-3 bg-blue-50 rounded text-center">
                            <p className="text-xs text-blue-600 mb-1">Retención 6s</p>
                            <p className="text-sm font-bold text-blue-800">{currentResult.analysis.engagementPrediction.retention_s6}</p>
                          </div>
                          <div className="p-3 bg-green-50 rounded text-center">
                            <p className="text-xs text-green-600 mb-1">Completado</p>
                            <p className="text-sm font-bold text-green-800">{currentResult.analysis.engagementPrediction.completion_rate}</p>
                          </div>
                          <div className="p-3 bg-purple-50 rounded text-center">
                            <p className="text-xs text-purple-600 mb-1">CTR</p>
                            <p className="text-sm font-bold text-purple-800">{currentResult.analysis.engagementPrediction.ctr}</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {currentResult.analysis.confidence && (
                      <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                        <p className="text-sm font-medium text-gray-600 mb-2">Confianza del Análisis</p>
                        <div className="flex items-center">
                          <div className="w-full bg-gray-200 rounded-full h-3 mr-3">
                            <div 
                              className="bg-gradient-to-r from-blue-500 to-purple-600 h-3 rounded-full" 
                              style={{ width: `${currentResult.analysis.confidence}%` }}
                            ></div>
                          </div>
                          <span className="text-lg font-bold text-gray-700">{currentResult.analysis.confidence}%</span>
                        </div>
                      </div>
                    )}
                  </div>
