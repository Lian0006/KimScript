    ...
    <section id="analysis-results" className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue="script" className="space-y-6">
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-6">
                <Card className="shadow-xl border-0 bg-white rounded-2xl overflow-hidden">
                  ...
                  <Collapsible open={openSections.viral} onOpenChange={() => toggleSection('viral')}>
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" className="w-full justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 hover:from-blue-100 hover:to-blue-200 rounded-xl border border-blue-200 transition-all duration-300">
                        <span className="font-semibold text-blue-800 flex items-center">
                        ...
                        {openSections.viral ? <ChevronUp className="h-5 w-5 text-blue-600" /> : <ChevronDown className="h-5 w-5 text-blue-600" />}
                      </Button>
                    </CollapsibleTrigger>
                    <CollapsibleContent className="mt-4">
                      <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-blue-200 space-y-6">
                        <div className="bg-white p-5 rounded-lg border border-blue-200 shadow-sm">
                          <p className="text-sm font-semibold text-blue-700 mb-4">Elementos Identificados</p>
                          <div className="flex flex-wrap gap-3">
                            {currentResult.analysis.viralElements.map((element, index) => (
                              <div key={index} className="bg-gradient-to-r from-slate-50 to-gray-50 border border-slate-300 rounded-lg px-4 py-3 shadow-sm">
                        ...
                        <div className="grid gap-4">
                          <div className="bg-white p-4 rounded-lg border border-blue-200">
                            <p className="text-sm font-medium text-blue-600 mb-2">Mecánicas Virales</p>
                            <p className="text-gray-800">{Array.isArray(currentResult.analysis.viralMechanics) ? currentResult.analysis.viralMechanics.join(', ') : currentResult.analysis.viralMechanics}</p>
                          </div>
                          {currentResult.analysis.contentFramework && (
                            <div className="bg-white p-4 rounded-lg border border-blue-200">
                              <p className="text-sm font-medium text-blue-600 mb-2">Framework Narrativo</p>
                              <p className="text-gray-800">
                                {typeof currentResult.analysis.contentFramework === 'string' 
                                  ? currentResult.analysis.contentFramework 
                                  : `${currentResult.analysis.contentFramework.primary} (${Math.round(currentResult.analysis.contentFramework.confidence * 100)}% confianza)`
                                }
                              </p>
                            </div>
                          )}
                          {currentResult.analysis.keyPhrases && currentResult.analysis.keyPhrases.length > 0 && (
                            <div className="bg-white p-4 rounded-lg border border-blue-200">
                              <p className="text-sm font-medium text-blue-600 mb-3">Frases Clave</p>
                              <div className="flex flex-wrap gap-2">
                                {currentResult.analysis.keyPhrases.map((phrase, index) => (
                                  <Badge key={index} variant="outline" className="border-blue-300 text-blue-700 text-xs">
                                    {typeof phrase === 'string' ? phrase : phrase.quote}
                                  </Badge>
                                ))}
                              </div>
                            </div>
                  ...
                  {currentResult.analysis.psychologicalTriggers && (
                    <Collapsible open={openSections.psychology} onOpenChange={() => toggleSection('psychology')}>
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" className="w-full justify-between p-4 bg-gradient-to-r from-violet-50 to-violet-100 hover:from-violet-100 hover:to-violet-200 rounded-xl border border-violet-200 transition-all duration-300">
                          ...
                          {openSections.psychology ? <ChevronUp className="h-5 w-5 text-violet-600" /> : <ChevronDown className="h-5 w-5 text-violet-600" />}
                        </Button>
                      </CollapsibleTrigger>
                      <CollapsibleContent className="mt-4">
                        <div className="bg-gradient-to-br from-violet-50 to-violet-100 p-6 rounded-xl border border-violet-200 space-y-6">
                          <div className="grid gap-4">
                            <div className="bg-white p-4 rounded-lg border border-violet-200">
                              <p className="text-sm font-medium text-violet-600 mb-2">Triggers Psicológicos</p>
                            ...
                            {currentResult.analysis.targetAudience && (
                              <div className="bg-white p-4 rounded-lg border border-violet-200">
                                <p className="text-sm font-medium text-violet-600 mb-2">Audiencia Objetivo</p>
                                <p className="text-gray-800">{currentResult.analysis.targetAudience}</p>
                              </div>
                            )}
                            <div className="bg-white p-4 rounded-lg border border-violet-200">
                              <p className="text-sm font-medium text-violet-600 mb-2">Tono Emocional</p>
                  ...
                  {(currentResult.analysis.viralPotential || currentResult.analysis.engagementPrediction) && (
                    <Collapsible open={openSections.predictions} onOpenChange={() => toggleSection('predictions')}>
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" className="w-full justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 hover:from-blue-100 hover:to-blue-200 rounded-xl border border-blue-200 transition-all duration-300">
                          <span className="font-semibold text-blue-800 flex items-center">
                          ...
                          {openSections.predictions ? <ChevronUp className="h-5 w-5 text-blue-600" /> : <ChevronDown className="h-5 w-5 text-blue-600" />}
                        </Button>
                      </CollapsibleTrigger>
                      <CollapsibleContent className="mt-4">
                        <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-blue-200 space-y-6">
                          <div className="grid gap-4">
                            {currentResult.analysis.viralPotential && (
                              <div className="bg-white p-4 rounded-lg border border-blue-200">
                                <p className="text-sm font-medium text-blue-600 mb-2">Potencial Viral</p>
                                <p className="text-gray-800">{currentResult.analysis.viralPotential}</p>
                              </div>
                            )}
                            {currentResult.analysis.engagementPrediction && (
                              <div className="bg-white p-4 rounded-lg border border-blue-200">
                                <p className="text-sm font-medium text-blue-600 mb-2">Predicción de Engagement</p>
                                <div className="space-y-2">
                                  <p className="text-sm text-gray-700">
                                    <span className="font-medium">Retención (6s):</span> {currentResult.analysis.engagementPrediction.retention_s6}
                                  </p>
                                  <p className="text-sm text-gray-700">
                                    <span className="font-medium">Tasa de Completado:</span> {currentResult.analysis.engagementPrediction.completion_rate}
                                  </p>
                                  <p className="text-sm text-gray-700">
                                    <span className="font-medium">CTR:</span> {currentResult.analysis.engagementPrediction.ctr}
                                  </p>
                                </div>
                              </div>
                            )}
                  ...
                  {currentResult.analysis.improvementRecommendations && currentResult.analysis.improvementRecommendations.length > 0 && (
                    <Collapsible open={openSections.recommendations} onOpenChange={() => toggleSection('recommendations')}>
                      <CollapsibleTrigger asChild>
                        <Button variant="ghost" className="w-full justify-between p-4 bg-gradient-to-r from-violet-50 to-violet-100 hover:from-violet-100 hover:to-violet-200 rounded-xl border border-violet-200 transition-all duration-300">
                          ...
                          {openSections.recommendations ? <ChevronUp className="h-5 w-5 text-violet-600" /> : <ChevronDown className="h-5 w-5 text-violet-600" />}
                        </Button>
                      </CollapsibleTrigger>
                      <CollapsibleContent className="mt-4">
                        <div className="bg-gradient-to-br from-violet-50 to-violet-100 p-6 rounded-xl border border-violet-200">
                          <div className="space-y-3">
                            {currentResult.analysis.improvementRecommendations.map((recommendation, index) => (
                              <div key={index} className="bg-white p-4 rounded-lg border border-violet-200 flex items-start">
                                <div className="w-6 h-6 bg-violet-600 rounded-full flex items-center justify-center mr-3 mt-0.5 flex-shrink-0">
          ...
          <div className="space-y-6">
            {/* New Analysis Panel */}
            <Card className="shadow-xl border-0 bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl overflow-hidden">
              <CardContent className="p-0">
                <div className="bg-gradient-to-r from-emerald-600 to-green-600 text-white p-6">
                  <h3 className="text-xl font-bold">Nuevo Análisis</h3>
                  <p className="text-emerald-100 text-sm mt-1">Crea un análisis fresco</p>
                </div>
                <div className="p-6">
                  <Button 
                    onClick={() => {
            ...
            <Card className="shadow-xl border-0 bg-gradient-to-br from-white to-gray-50 rounded-2xl overflow-hidden">
              <CardContent className="p-0">
                <div className="bg-gradient-to-r from-violet-600 to-blue-600 text-white p-6">
                  <h3 className="text-xl font-bold">Exportar y Compartir</h3>
                  <p className="text-violet-100 text-sm mt-1">Gestiona tus resultados</p>
                </div>
        ...
        {/* Script Generation Section - Full Width Below Analysis */}
        <div className="mb-8">
          {!currentResult.generatedScript ? (
            <div className="bg-gradient-to-r from-violet-50 to-blue-50 rounded-lg p-1">
              <AdvancedScriptGenerator 
                analysisId={currentResult.id} 
                onScriptGenerated={handleScriptGenerated}
              />
            ...
            <Card className="shadow-xl border-0 bg-gradient-to-br from-white to-gray-50 rounded-2xl overflow-hidden">
              <CardContent className="p-0">
                <div className="bg-gradient-to-r from-violet-600 to-blue-600 text-white p-8 text-center">
                  ...
                  <p className="text-violet-100 text-lg">Guión viral generado con técnicas neurológicas avanzadas</p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-900 text-center">Hook Viral</h4>
                    <div className="p-4 bg-blue-50 border-l-4 border-blue-600 rounded-r-lg min-h-[100px] flex items-center">
                      <p className="text-gray-900 font-medium text-center w-full">{currentResult.generatedScript.hook}</p>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-900 text-center">Cuerpo Principal</h4>
                  ...
                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-900 text-center">Call to Action</h4>
                    <div className="p-4 bg-violet-50 border-l-4 border-violet-600 rounded-r-lg min-h-[100px] flex items-center">
                      <p className="text-gray-900 font-medium text-center w-full">{currentResult.generatedScript.cta}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 grid md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    <h4 className="font-semibold text-gray-900">Emociones a Evocar:</h4>
                    <div className="flex flex-wrap gap-2">
                      {currentResult.generatedScript.emotions.map((emotion, index) => (
                        <Badge key={index} variant="secondary" className="bg-blue-100 text-blue-700">
                          {emotion}
                  ...
                  {currentResult.generatedScript.visualSuggestions && currentResult.generatedScript.visualSuggestions.length > 0 && (
                    <div className="space-y-3">
                      <h4 className="font-semibold text-gray-900">Sugerencias Visuales:</h4>
                      <ul className="space-y-1">
                        {currentResult.generatedScript.visualSuggestions.map((suggestion, index) => (
                          <li key={index} className="text-sm text-gray-700 flex items-start">
                            <span className="text-violet-600 mr-2 mt-1">•</span>
                ...
                {/* Technical Script Section */}
                {currentResult.generatedScript.technicalScript && (
                ...
                {/* Viral Adaptation Details Section */}
                {currentResult.generatedScript.adaptationDetails && (
                  <div className="mt-6 p-4 bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg border border-green-200">
                    <h4 className="font-semibold text-gray-900 mb-4 flex items-center">
                      ...
                      Adaptación de Elementos Virales
                    </h4>
                    <div className="space-y-4">
                      <div className="bg-white p-4 rounded border border-green-200">
                        ...
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.hookAdaptation}
                        </p>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded border border-green-200">
                        ...
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.toneAdaptation}
                        </p>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded border border-green-200">
                        <h5 className="font-medium text-green-700 mb-2 flex items-center">
                          <span className="w-2 h-2 bg-green-600 rounded-full mr-2"></span>
                          Adaptación del Lenguaje/Apodos
                        </h5>
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.languageAdaptation}
                        </p>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded border border-green-200">
                        <h5 className="font-medium text-green-700 mb-2 flex items-center">
                          <span className="w-2 h-2 bg-green-600 rounded-full mr-2"></span>
                          Adaptación del Framework Narrativo
                        </h5>
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.narrativeAdaptation}
                        </p>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded border border-green-200">
                        <h5 className="font-medium text-green-700 mb-2 flex items-center">
                          <span className="w-2 h-2 bg-green-600 rounded-full mr-2"></span>
                          Adaptación de Frases Clave
                        </h5>
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.keyPhrasesAdaptation}
                        </p>
                      </div>
                      ...
                      <div className="bg-white p-4 rounded border border-green-200">
                        <h5 className="font-medium text-green-700 mb-2 flex items-center">
                          <span className="w-2 h-2 bg-green-600 rounded-full mr-2"></span>
                          Adaptación del Call to Action
                        </h5>
                        <p className="text-sm text-gray-700 leading-relaxed">
                          {currentResult.generatedScript.adaptationDetails.ctaAdaptation}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {currentResult.generatedScript.viralHashtags && currentResult.generatedScript.viralHashtags.length > 0 && (
                  <div className="mt-6 p-4 bg-violet-50 rounded-lg border border-violet-200">
                    ...
                    <div className="bg-white p-4 rounded border">
                      <div className="flex flex-wrap gap-2">
                        {currentResult.generatedScript.viralHashtags.map((hashtag, index) => (
                          <Badge key={index} className="bg-violet-600 text-white hover:bg-violet-700 cursor-pointer" 
                                onClick={() => navigator.clipboard.writeText(hashtag)}>
                            {hashtag}
                          </Badge>
                        ))}
                      </div>
                      <p className="text-xs text-gray-500 mt-2">Haz clic en cualquier hashtag para copiarlo</p>
                    </div>
                  </div>
                )}

                <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                  <h4 className="font-semibold text-gray-900 mb-2">Tono de Voz Recomendado:</h4>
                  <p className="text-sm text-gray-700">{currentResult.generatedScript.toneOfVoice}</p>
                </div>
